...
export class LearningPage {
    ...
    async render() {
        ...
        this.view.viewContainer.empty();
        const learningContainer = this.view.viewContainer.createDiv({ cls: 'openwords-learning-container' });
    ...
    async pickNextLCard() {
        ...
        let pool: CardInfo[];
        if (this.view.page === 'new') {  // 新词排序：间隔降序，间隔相同则易记因子升序
            pool = Array.from(this.view.plugin.newCards.values());
            if (pool.length === 0) {
                new Notice("已完成所有新词！");
                this.view.page = 'home';
                await this.view.render();
            ...
            pool = Array.from(this.view.plugin.dueCards.values())
        ...
        const randomMode = Math.random() < this.view.plugin.settings.randomRatio;
        if (randomMode) {
    ...
    renderCard(cardContainer: HTMLDivElement) {
        ...
        const file = this.view.plugin.app.vault.getFileByPath(this.currentLearingCard.path);
        if (!file) { return; }
        const fileCache = this.view.plugin.app.metadataCache.getFileCache(file);
    ...
    // 评分按钮渲染
    renderSettings( cardContainer: HTMLDivElement, settingsContainer: HTMLElement) {
    ...
    // 数字键评分
    registerRatingKeyEvents( learningContainer: HTMLDivElement, cardContainer: HTMLDivElement) {
        ...
        this.view.plugin.registerDomEvent(learningContainer, 'keydown', handleKeydown);
        this.view.plugin.registerDomEvent(learningContainer, 'keyup', handleKeyup);
    }

    // 答案渲染逻辑
    registerRenderKeyEvents( learningContainer: HTMLDivElement, cardContainer: HTMLDivElement) {
        let isShowingMarkdown = false;
        const showMarkdown = async () => {
            if (isShowingMarkdown || !this.currentLearingCard) return;
            const file = this.view.plugin.app.vault.getFileByPath(this.currentLearingCard.path);
            if (file instanceof TFile) {
                const markdownContent = await this.view.plugin.app.vault.cachedRead(file);
                cardContainer.empty();
